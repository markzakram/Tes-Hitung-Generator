...
import { fmt } from './num';
import { buatOpsi } from './distractors';
...
function buatSoal(
  ...
  const pg = opsi.pilihanGanda
    ? buatOpsi(k.node, k.nilai, rnd, opsi.jumlahOpsi, opsi.urutkanOpsi)
...
export function generatePaket(
  ...
  for (const blok of BLOK) {
    ...
    for (let i = 0; i < slot; i++) {
      soal.push(
        buatSoal(blok.dari + i, blok, rnd, opsi, rencana[i], pool, dipakaiGlobal, dipakaiPaket),
      );
    }
  }
